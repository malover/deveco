...
<check if="project-scan-report.json exists">
  ...
  <action>Extract cached_llm_analysis and cached_baseline from state file if present (hybrid LLM+CSV approach)</action>
...
<ask>I found an in-progress workflow state from {{last_updated}}.
    ...
    **Current Progress:**
    ...
    - Project Type(s): {{analysis_summary}} (LLM-driven detection)
  ...
  <check if="user selects 1">
    ...
    <action>Load cached_llm_analysis from state file</action>
    <action>Load cached_baseline from state file</action>

    <critical>HYBRID RESUME: LLM analysis is cached — no re-analysis needed.
    Baseline CSV rows are loaded from cache, not from file (prevents CSV drift).</critical>

    <action>Display: "Resuming {{workflow_mode}} from {{current_step}}.
    LLM analysis cached with confidence: {{analysis_confidence}}.
    Parts: {{parts_count}} ({{parts_list}})"</action>
