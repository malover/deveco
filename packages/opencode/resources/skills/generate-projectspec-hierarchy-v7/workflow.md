...
Read `references/document-model.md`, `references/business.md`, `references/constraints-and-limitations.md`, and for large repositories `references/evidence-and-performance.md`.
...
4. Write **Extension and Modification Points**: where new behavior belongs, reusable seams, and existing patterns to follow.
5. Link the applicable Project/workspace `constraints-and-limitations.md`; do not repeat its rules, limitations, or change checks in Architecture.

### 3.3 Write the Project constraints and limitations registry

After the Project Architecture/module pass, load `templates/constraints-and-limitations.md` and consolidate all evidenced governance for that Project:

1. assign stable `ARC-*`, `LIM-*`, and `CHK-*` IDs;
2. scope entries to the Project, module/build unit, contract, data domain, or native boundary;
3. classify constraint basis as `Enforced`, `Declared policy`, `Observed pattern`, or `Inferred guardrail`;
4. add impact/blast radius, exact evidence, and a verification/check method;
5. distinguish verified limitations from tooling/external gaps and local evidence not inspected;
6. deduplicate repeated Architecture-level statements in an applicability matrix;
7. preserve developer-maintained content outside generated markers.

Do not create module constraint files. Do not merely concatenate old sections: the registry must add basis, scope, verification, stable limitation IDs, and traceability.
...
- Project Architecture inventories every module/build unit and links the Project governance registry.
- Project `constraints-and-limitations.md` owns Project and module-scoped governance.
- Child documents link to the Project pair; Project documents link to children and the root.

### Optional OpenHarmony subsystem

Generate a subsystem Architecture roll-up only when authoritative component/subsystem mapping exists and at least two components make it useful. Keep governance in the workspace/Project registries and link applicable IDs. Do not invent a subsystem Business owner.

### High level

After all Project summaries:

1. Write high-level Architecture from Project boundaries, cross-Project contracts, and shared data/integration/lifecycle concerns.
2. In multi-project mode write root `constraints-and-limitations.md` containing only governance whose blast radius crosses Projects; never copy Project-local entries upward.
3. Write high-level Business from the capability portfolio and complete cross-Project journeys. Do not reduce it to links.
4. Write `index.md` last as a concise access map; substance remains in Business/Architecture/governance documents.
...
- Are governance entries scoped, evidence-calibrated, deduplicated, and verifiable?
- Are Architecture documents free of relocated constraint/limitation/guardrail sections?
...
3. Reanalyze changed owners, impacted flows/consumers, and applicable governance entries.
4. Regenerate affected regions plus ancestor summaries, governance traceability, and index links.
