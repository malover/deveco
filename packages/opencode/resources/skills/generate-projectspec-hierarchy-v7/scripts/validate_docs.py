#!/usr/bin/env python3
"""Validate ProjectSpec v7 Markdown, frozen inventory, and documentation plan."""
...
RELOCATED_ARCHITECTURE_HEADINGS = {
...
GOVERNANCE_REQUIRED = {
    "applicability and inheritance": r"^##\s+Applicability and Inheritance\s*$",
    "architectural constraints and invariants": r"^##\s+Architectural Constraints and Invariants\s*$",
    "known limitations and evidence gaps": r"^##\s+Known Limitations and Evidence Gaps\s*$",
    "change checks": r"^##\s+Change Checks\s*$",
    "architecture and capability traceability": r"^##\s+Architecture and Capability Traceability\s*$",
    "evidence register": r"^##\s+Evidence Register\s*$",
}
GOVERNANCE_BASIS_RE = re.compile(r"\b(?:Enforced|Declared policy|Observed pattern|Inferred guardrail)\b", re.I)
BUSINESS_REQUIRED = {
...
def validate_file(path: Path, docs_root: Path) -> tuple[list[str], str | None]:
    ...
    if "architecture" in path.name.casefold():
        for label, pattern in RELOCATED_ARCHITECTURE_HEADINGS.items():
            if re.search(pattern, text, re.MULTILINE | re.I):
                errors.append(f"{rel}: relocated governance section remains in Architecture: {label}")
        if "constraints-and-limitations.md" not in text:
            errors.append(f"{rel}: Architecture does not link its constraints-and-limitations.md registry")

    if path.name.casefold() == "constraints-and-limitations.md":
        if "modules" in {part.casefold() for part in rel.parts[:-1]}:
            errors.append(f"{rel}: module-level governance files are not allowed; scope entries in the Project registry")
        for label, pattern in GOVERNANCE_REQUIRED.items():
            body = section_body(text, pattern)
            if body is None:
                errors.append(f"{rel}: missing mandatory governance section: {label}")
            elif len(re.sub(r"[\s|#*`-]+", "", body)) < 40:
                errors.append(f"{rel}: governance section is too thin: {label}")
        constraints = section_body(text, GOVERNANCE_REQUIRED["architectural constraints and invariants"])
        limitations = section_body(text, GOVERNANCE_REQUIRED["known limitations and evidence gaps"])
        checks = section_body(text, GOVERNANCE_REQUIRED["change checks"])
        if constraints and "ARC-" in constraints and not GOVERNANCE_BASIS_RE.search(constraints):
            errors.append(f"{rel}: constraints do not classify basis/strength")
        if constraints and "ARC-" in constraints and not re.search(r"\bVerification(?:/check)?\b", constraints, re.I):
            errors.append(f"{rel}: constraints do not include verification/check information")
        if limitations and not re.search(r"\bLIM-[A-Za-z0-9._-]+\b|No (?:additional |cross-Project )?limitations", limitations, re.I):
            errors.append(f"{rel}: limitations need stable LIM-* IDs or an explicit evidence-backed none statement")
        if checks and not re.search(r"\bCHK-[A-Za-z0-9._-]+\b|No (?:additional |cross-Project )?change checks", checks, re.I):
            errors.append(f"{rel}: change checks need stable CHK-* IDs or an explicit evidence-backed none statement")
...
def governance_texts(docs_root: Path) -> dict[Path, str]:
    return {
        path: path.read_text(encoding="utf-8")
        for path in docs_root.rglob("constraints-and-limitations.md")
    }
...
def validate_plan(docs_root: Path, plan: dict[str, Any]) -> list[str]:
    ...
    governance = governance_texts(docs_root)
    ...
    for project in plan.get("projects", []):
        ...
        governance_document = project.get("governanceDocument")
        if not isinstance(governance_document, str) or not governance_document.strip():
            errors.append(f"plan: Project {project_id} lacks governanceDocument")
        else:
            normalized_governance = normalized_document(governance_document)
            governance_path = docs_root / normalized_governance
            if normalized_governance not in planned:
                errors.append(f"plan: Project {project_id} governance document is not listed in plan.documents: {governance_document}")
            if governance_path not in governance:
                errors.append(f"plan: Project {project_id} governance document does not exist: {governance_document}")
...
def validate_tree(
    ...
    for required in ("index.md", "high-level-business.md", "high-level-architecture.md", "constraints-and-limitations.md"):
...
def write_self_test_fixture(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    (root / "index.md").write_text(
        f"{START}\n# Index\n\n[Business](high-level-business.md) · [Architecture](high-level-architecture.md) · [Governance](constraints-and-limitations.md)\n{END}\n",
        encoding="utf-8",
    )
    ...
    (root / "high-level-architecture.md").write_text(
        ...
        "## Architecture summary\n\nThe owner coordinates the supporting module through its public contract.\n"
        f"{END}\n",
        encoding="utf-8",
    )
    (root / "constraints-and-limitations.md").write_text(
        f"{START}\n# Constraints and Limitations — Fixture\n\n> Scope ID: _root\n\n"
        "## Applicability and Inheritance\n\n| Scope/unit | Applicable ARC IDs | Relevant LIM IDs | Required CHK IDs | Architecture detail |\n|---|---|---|---|---|\n| Project | ARC-TEST-01 | LIM-TEST-01 | CHK-TEST-01 | high-level architecture |\n\n"
        "## Architectural Constraints and Invariants\n\n| ID | Scope | Constraint/invariant | Basis | Why/impact and blast radius | Evidence | Verification/check |\n|---|---|---|---|---|---|---|\n| ARC-TEST-01 | Project | Calls use the owner contract. | Observed pattern | Keeps state mutation in one owner. | `owner/run#complete` | Inspect callers of the owner contract. |\n\n"
        "## Known Limitations and Evidence Gaps\n\n| ID | Scope | Category | Limitation or exact evidence gap | Current impact | Evidence | Current handling / exact unknown |\n|---|---|---|---|---|---|---|\n| LIM-TEST-01 | Project | External evidence unavailable | Platform rendering is outside the fixture. | Rendering remains unverified. | external boundary | Exact platform result is unknown. |\n\n"
        "## Change Checks\n\n| ID | Scope | Applies when | Required inspection/build/test check | Related ARC/LIM IDs | Evidence |\n|---|---|---|---|---|---|\n| CHK-TEST-01 | Project | Owner contract changes | Inspect consumers and run route/result/error checks. | ARC-TEST-01 | `owner/run#complete` |\n\n"
        "## Architecture and Capability Traceability\n\n| ARC/LIM/CHK ID | Architecture owner | Affected capability/flow | Consumers/blast radius |\n|---|---|---|---|\n| ARC-TEST-01 | high-level architecture | CAP-browse / FLOW-browse | owner and support |\n\n"
        "## Evidence Register\n\n| Claim/entry | Evidence anchor | Class | What it proves | What it does not prove |\n|---|---|---|---|---|\n| ARC-TEST-01 | `owner/run#complete` | Observed | Owner contract completes. | Platform rendering. |\n"
        f"{END}\n",
        encoding="utf-8",
    )
    inventory = {
    ...
    plan = {
        "documents": ["index.md", "high-level-business.md", "owner-business.md", "support-business.md", "high-level-architecture.md", "constraints-and-limitations.md"],
        "projects": [{"id": "_root", "governanceDocument": "constraints-and-limitations.md", "modules": [
...
def self_test() -> int:
    ...
    cases: list[tuple[str, str, str, str]] = [
        ...
        ("relocated Architecture section", "high-level-architecture.md", "## Architecture summary", "## Architectural Constraints and Invariants\n\nDuplicated rule.\n\n## Architecture summary"),
        ("missing constraint basis", "constraints-and-limitations.md", "Observed pattern", "Repository convention"),
        ("missing constraint verification", "constraints-and-limitations.md", "Verification/check", "Validation method"),
    ]
    ...
    plan_cases: list[tuple[str, Any, str]] = [
        ...
        ("missing governance document", lambda plan: plan["projects"][0].pop("governanceDocument"), "lacks governanceDocument"),
    ]
