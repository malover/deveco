...
- [Architecture specification](high-level-architecture.md) — ownership, entry points, runtime/data flows, dependencies, and extension seams.
- [Constraints and limitations](constraints-and-limitations.md) — scoped invariants, limitations/evidence gaps, and required change checks.

## Find the right owner for a change

| Goal/change area | Business capability/process | Architecture owner | Applicable governance IDs | Tests/evidence |
|---|---|---|---|
...
- Before feature work: start from the capability, read the owning Architecture, then resolve applicable governance IDs.
- Before changing a shared contract/data owner: inspect listed consumers and blast radius.
- During review: verify affected flows/rules, applicable `ARC-*`/`LIM-*`/`CHK-*`, tests, and documentation links.
