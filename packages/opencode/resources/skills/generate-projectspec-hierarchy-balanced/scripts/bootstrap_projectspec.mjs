...
async function main() {
  ...
  const scanReport = { schemaVersion: 2, mode: previousReport.mode || "manual-or-goal-step0", scanLevel, repositoryRevision: revision, homegraphRevision: previousReport.homegraphRevision || null, completedSteps: [...new Set(["bootstrap", ...priorCompleted])], cachedLLMAnalysis: previousReport.cachedLLMAnalysis || null, mergedCSVBaseline: previousReport.mergedCSVBaseline || { source: "documentation-requirements.csv", status: "pending" }, currentStep: previousReport.currentStep || "homegraph-readiness", outputs: [".projectspec/workspace-inventory.json", ".projectspec/documentation-plan.json", ".projectspec/evidence-plan.json"], missingOutputs: documents.map((item) => item.path), validationStatus: "pending", resume: { reusedCheckpoint: Boolean(previousReport.schemaVersion === 2 && previousReport.selectedRevision === revision), changedModules: changed, impactedAncestors: [...new Set(changed.map((id) => id.split("@", 1)[0]))] }, homegraph: previousReport.homegraph || { readiness: "pending", anchors: [], projectModuleCoverage: [], representativeFlows: [], stateDataPersistenceIntegration: [], unresolvedQuestions: [], recoveryAttempts: [], fallbackApproval: null } }
