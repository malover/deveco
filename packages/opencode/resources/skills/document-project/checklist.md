...
- [ ] UX flows documented (primary flow, secondary flows, error/offline flows) with state-transition diagrams
- [ ] Navigation model clear and complete (all ViewStates, back-press paths, transition triggers)
- [ ] Component interaction map present (action → component → ViewModel method → state change)
- [ ] Conditional rendering table (ViewState → rendered/hidden components)
- [ ] UX flow document references screen trees and wireframes via cross-links
- [ ] Design tokens extracted from resource files (colors, typography, spacing, shadows, backgrounds/gradients, icon sizes)
- [ ] UX wireframes generated (ux-screen-wireframes.html) with proper layout, colors, and annotation tooltips
- [ ] Interactive UX mockup generated (ux-interactive-mockup.html) implementing full state machine from ux-flows with clickable phone-frame, live state panel, and navigation history
- [ ] Icon/asset images copied from project resources and embedded as &lt;img&gt; tags in wireframe HTML
- [ ] Icon assets inventory table (resource name → file → component) included in screen tree doc
...
- [ ] Translation file structure documented (paths, formats, naming conventions)
- [ ] Translation key patterns and namespaces explained
- [ ] Fallback chain and locale resolution logic documented
- [ ] Missing/incomplete translation handling described
- [ ] How to add a new locale documented with step-by-step instructions
