...
class LocalCodeAttributionTracker implements CodeAttributionTracker {
  ...
  private async snapshotCurrentWorktree(source: AttributionSource): Promise<void> {
    ...
    for (const file of await this.repository.dirtyPaths()) {
      files[file] = createAttributedFile(await this.repository.readWorktree(file), source)
    }
    ledger.files = files
  }
  ...
  private async reconcileWorktree(source: AttributionSource): Promise<void> {
    ...
    const files: AttributionLedger["files"] = {}
    for (const file of await this.repository.dirtyPaths()) {
      const content = await this.repository.readWorktree(file)
      const previous =
        ledger.files[file] ??
        createAttributedFile(
          ledger.head ? await this.repository.readBlob(ledger.head, file) : "",
          ATTRIBUTION_SOURCE.UNKNOWN,
        )
      files[file] = attributeContent(previous, content, source)
    }
    ledger.files = files
    await this.save()
  }
  ...
  private async resetForHistoryTransition(head: string, source: AttributionSource): Promise<void> {
    ...
    if (!ledger) return
    ledger.head = head
    await this.snapshotCurrentWorktree(source)
  }
  ...
  private async catchUp(fallback: AttributionSource): Promise<void> {
    ...
    if (size < ledger.reflogOffset) {
      ...
      await this.snapshotCurrentWorktree(ATTRIBUTION_SOURCE.UNKNOWN)
      await this.save()
      return
    }
  ...
  private async processCommit(entry: GitReflogEntry, fallback: AttributionSource): Promise<boolean> {
    ...
    const patch = await this.repository.patchId(parent, entry.newHead)
    ...
    for (const change of await this.repository.commitChanges(compareBase, entry.newHead)) {
  ...
  private async refreshFilesAfterCommit(fallback: AttributionSource): Promise<void> {
    ...
    for (const file of await this.repository.dirtyPaths()) {
      ...
      files[file] = attributeContent(previous, content, fallback)
    }
    ledger.files = files
  }
}
