...
const ATTRIBUTION_STORAGE_DIRECTORY = "code-attribution"
...
export class AttributionLedgerStorage {
  ...
  constructor(repositoryRoot: string, dataDir = Global.Path.data) {
    ...
    this.filePath = path.join(dataDir, "analytics", ATTRIBUTION_STORAGE_DIRECTORY, `${key}.json`)
...
export async function clearAttributionLedgers(dataDir = Global.Path.data): Promise<void> {
  await fs.rm(path.join(dataDir, "analytics", ATTRIBUTION_STORAGE_DIRECTORY), { recursive: true, force: true })
}
