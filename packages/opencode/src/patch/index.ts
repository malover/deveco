  ...
  for (const hunk of hunks) {
    switch (hunk.type) {
      case "add": {
        ...
        yield* Effect.logInfo(`Added file: ${sanitizePath(hunk.path)}`)
        break
      }
      ...
      case "delete": {
        ...
        yield* Effect.logInfo(`Deleted file: ${sanitizePath(hunk.path)}`)
        break
      }
      ...
      case "update": {
        ...
        if (hunk.move_path) {
          ...
          yield* Effect.logInfo(`Moved file: ${sanitizePath(hunk.path)} -> ${sanitizePath(hunk.move_path)}`)
        } else {
          yield* fs.writeWithDirs(hunk.path, Bom.join(fileUpdate.content, fileUpdate.bom))
          modified.push(hunk.path)
          yield* Effect.logInfo(`Updated file: ${sanitizePath(hunk.path)}`)
        }
        break
      }
    }
